...
def main(opt, callbacks=Callbacks()):
    ...
    # WandB와 연동하여 sweep config를 가져오기
    # wandb.init() 후에 opt를 업데이트해야 합니다.
    # sweep으로 지정된 모든 파라미터는 wandb.config에 저장됩니다.
    # 따라서 opt 객체의 속성을 wandb.config에서 가져온 값으로 덮어씁니다.
    # hyp 파라미터는 특수하게 처리됩니다.

    # hyp.yaml 파일 경로를 먼저 처리합니다.
    # opt.hyp는 Sweep YAML에서 "hyp" 파라미터 블록으로 정의되어 있으므로,
    # 이는 hyp.yaml 파일 경로가 아니라, wandb.config의 하이퍼파라미터 딕셔너리가 됩니다.
    # 따라서, opt.hyp = wandb.config.hyp 로 설정해야 합니다.

    # WandB config에서 값 가져오기
    # 이때, parse_opt에서 파싱된 opt가 먼저 생성되고, 그 후에 wandb.config의 값이 덮어씌워져야 합니다.
    # 이렇게 하면 wandb config에 없는 파라미터는 opt의 기본값을 사용합니다.

    # 먼저 opt를 파싱합니다.
    # opt = parse_opt(known=True) # 이미 if __name__ == "__main__": 블록에서 처리되므로 여기서는 호출하지 않아도 됩니다.

    # WandB 초기화
    # name, project, entity는 sweep 설정에 따라 자동으로 설정되거나,
    # wandb.init 호출 시 명시적으로 전달할 수 있습니다.
    # 여기서는 Sweep에서 설정된 project와 entity를 사용하도록 합니다.
    wandb.init(project=opt.project, entity=opt.entity, name=opt.name)

    # WandB Config를 opt 객체에 반영
    # hyp는 딕셔너리 형태로 직접 전달되므로 별도 처리
    # 다른 모든 파라미터는 wandb.config에서 가져와 opt에 설정합니다.
